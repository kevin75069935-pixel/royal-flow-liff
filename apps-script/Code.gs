...
function defaultCarTypeRows() {
  return [
    ...
    carTypeRow("Benz Sprinter 旗艦8人座", 3600, 65, 12, 1500, "Y", "大型旗艦車"),
    carTypeRow("VW Crafter 旗艦8人座", 3400, 62, 12, 1500, "Y", "大型旗艦車"),
...
function defaultBusinessHourlyRows() {
  return [
    ...
    businessHourlyRow("Benz Sprinter 旗艦8人座", "是", "雙北市區", 6000, 9500, 1500, "是", "高端大型保母車"),
    businessHourlyRow("VW Crafter 旗艦8人座", "是", "雙北市區", 6000, 9500, 1500, "是", "高端大型保母車"),
...
function findCarPrice(carTypes, carType) {
  ...
  const selectedAliases = carTypeAliases(selected);
  for (let i = 0; i < carTypes.length; i++) {
    const rowType = value(carTypes[i].car_type);
    const rowAliases = carTypeAliases(rowType);
    if (selected && (
      selected === rowType ||
      selected.indexOf(rowType) >= 0 ||
      rowType.indexOf(selected) >= 0 ||
      selectedAliases.some(function(alias) { return rowAliases.indexOf(alias) >= 0; })
    )) {
      return carTypes[i];
    }
  }
...
function findBusinessHourlyPrice(rows, carType) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const rowType = value(rows[i].car_type);
    const rowAliases = carTypeAliases(rowType);
    if (selected && (
      selected === rowType ||
      selected.indexOf(rowType) >= 0 ||
...
function carTypeAliases(carType) {
  const text = value(carType);
  const aliases = [text];
  if (/Sprinter/i.test(text)) {
    aliases.push("Benz Sprinter 旗艦8人座", "Benz Sprinter 旗艦9人座", "Mercedes-Benz Sprinter");
  }
  if (/Crafter/i.test(text)) {
    aliases.push("VW Crafter 旗艦8人座", "VW Crafter 旗艦9人座");
  }
    ...
    if (matchedOrderId) {
      ...
      if (ordersSheet) {
        ...
        if (order) {
          ...
          customerLineUserId = order.line_user_id || "";
      ...
      if (ordersSheet) {
        ...
        if (recentOrder) {
          ...
          customerLineUserId = recentOrder.line_user_id || "";
